...
export function interleaveWith<T>(
  createPlaceholder: (i: number) => T,
  items: T[]
) {
  return items.flatMap((item, i) =>
    item === last(items) ? [item] : [item, createPlaceholder(i)]
  );
}
 ...
 */
export function calcShrinkMask(
  widths: number[],
  ...
  for (let shrinkCount = 1; shrinkCount <= widths.length; shrinkCount++) {
    const foundComb = findComb(indexes, shrinkCount, (comb) => {
      const compressedWidths = widths.map((width, i) =>
        comb.includes(i) ? minWidth : width
      );
