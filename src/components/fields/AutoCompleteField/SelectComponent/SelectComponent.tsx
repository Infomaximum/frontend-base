...
class _Select extends React.PureComponent<ISelectComponentProps, ISelectState> {
  ...
  public override componentDidUpdate(
    prevProps: Readonly<ISelectComponentProps>
  ): void {
  ...
  private getSelectValue = createSelector(
    ...
    (_localization: Localization, fieldValue: IModel[] | undefined) =>
      fieldValue,
  ...
  private onSearch = () => {
    ...
    if (isSearchTextChanged && isString(searchText) && isDropdownOpened) {
      autocompleteStore.searchValueChange(
        searchText,
        mapExcludedData(value, queryVariables)
      );
    }
  };
  ...
  private getFieldValueCollection = createSelector(
    identity,
    (models: IModel[]) => {
  ...
  private mapSelectValueToModel(value: RawValueType) {
    ...
    return (
      autocompleteStore.list?.[value] ??
      this.getFieldValueCollection(fieldValue)[value]
    );
  }
  ...
  private handleChange = (
    selectStruct: LabeledValue | LabeledValue[] | undefined
  ): void => {
    const { onChange } = this.props;
    selectStruct = isArray(selectStruct)
      ? selectStruct
      : selectStruct
      ? [selectStruct]
      : [];
    onChange(
      compact(
        map(selectStruct, ({ value }) => this.mapSelectValueToModel(value))
      )
    );
  };

  private renderOptions(
    modelList: TDictionary<IModel> | IModel[]
  ): React.ReactNode[] {
    const {
