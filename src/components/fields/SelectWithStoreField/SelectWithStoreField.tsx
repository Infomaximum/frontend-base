...
const SelectWithStoreComponent: FC<ISelectWithStoreComponentProps> = memo(
  (props) => {
    const {
      readOnly,
      disabled,
    ...
    const handleChange = useCallback<
      NonNullable<ISelectWithStoreProps["onChange"]>
    >(
      (value) => {
    ...
    if (localization && readOnly && isNil(valueProps)) {
      return (
        <Input
          value={localization.getLocalized(NOT_SELECTED)}
          disabled={true}
        />
      );
    }
...
const SelectWithStoreField: React.FC<ISelectWithStoreFieldProps> = (props) => (
  <Field
    component={SelectWithStoreComponent}
    isEqual={isSameValue}
    {...props}
  />
);
