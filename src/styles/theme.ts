const colors = {
  /** =========================== Colors [START]  =========================== */
  transparentColor: "rgba(0, 0, 0, 0)",
  /** --------------------------------- Blue -------------------------------- */
  blue1Color: "#F0F8FF",
  blue15Color: "#E6F1FA",
  blue2Color: "#D1E9FF",
  blue3Color: "#A3CCF7",
  blue4Color: "#75AAEB",
  blue5Color: "#4B88DE",
  blue6Color: "#2667D1",
  blue7Color: "#164AAB",
  blue8Color: "#0B3185",
  blue9Color: "#031D5E",
  blue10Color: "#010F38",
  blue11Color: "#E2F2F1",
  /** --------------------------------- Azure -------------------------------- */
  azure1Color: "#18A0FB",
  azure2Color: "#1C84FF",
  azure3Color: "#1A72DA",
  azure4Color: "#1766C3",

  /** --------------------------------- Grey -------------------------------- */
  grey1Color: "#FFFFFF",
  grey2Color: "#FAFAFA",
  grey3Color: "#F5F5F5",
  grey4Color: "#F0F0F0",
  grey45Color: "#E6E6E6",
  grey5Color: "#D9D9D9",
  grey6Color: "#BFBFBF",
  grey65Color: "#ABABAB",
  grey7Color: "#8C8C8C",
  grey75Color: "#737373",
  grey8Color: "#595959",
  grey9Color: "#434343",
  grey10Color: "#262626",
  grey11Color: "#1f1f1f",
  grey12Color: "#141414",
  grey13Color: "#000000",
  grey14Color: "#5F6E78",
  /** --------------------------------- Red -------------------------------- */
  red1Color: "#FFF3F0",
  red2Color: "#FFE7E3",
  red3Color: "#FFC2BA",
  red4Color: "#F2938A",
  red5Color: "#E6655E",
  red6Color: "#D93A36",
  red7Color: "#B32426",
  red8Color: "#8C151B",
  red9Color: "#660A12",
  red10Color: "#40060D",
  /** --------------------------------- Volcano -------------------------------- */
  volcano1Color: "#FFF2E8",
  volcano2Color: "#FFD8BF",
  volcano3Color: "#FFBB96",
  volcano4Color: "#FF9C6E",
  volcano5Color: "#FF7A45",
  volcano6Color: "#FA541C",
  volcano7Color: "#D4380D",
  volcano8Color: "#AD2102",
  volcano9Color: "#871400",
  /** --------------------------------- Yellow -------------------------------- */
  yellow1Color: "#FEFFE6",
  yellow2Color: "#FFFFB8",
  yellow3Color: "#FFFB8F",
  yellow4Color: "#FFF566",
  yellow5Color: "#FFD98C",
  yellow6Color: "#F6C33E",
  yellow7Color: "#E0A200",
  yellow8Color: "#AD8B00",
  yellow9Color: "#876800",
  yellow10Color: "#614700",
  /** --------------------------------- Orange -------------------------------- */
  orange1Color: "#FFF7E6",
  orange2Color: "#FFE7BA",
  orange3Color: "#FFD591",
  orange4Color: "#FFC069",
  orange5Color: "#FFA940",
  orange6Color: "#FA8C16",
  orange7Color: "#D46B08",
  orange8Color: "#AD4E00",
  orange9Color: "#873800",
  orange10Color: "#612500",
  orange11Color: "#FAF0D9",
  /** --------------------------------- Gold -------------------------------- */
  gold1Color: "#FFFBE6",
  gold2Color: "#FFF1B8",
  gold3Color: "#FFE58F",
  gold4Color: "#FFD666",
  gold5Color: "#FFC53D",
  gold6Color: "#FAAD14",
  gold7Color: "#D48806",
  gold8Color: "#AD6800",
  gold9Color: "#874D00",
  gold10Color: "#613400",
  /** --------------------------------- Green -------------------------------- */
  green1Color: "#E8F5E9",
  green2Color: "#C8E6C9",
  green3Color: "#A5D6A7",
  green4Color: "#81C784",
  green5Color: "#66BB6A",
  // цвет для иконки type="check"
  green6Color: "#4CAF50",
  green7Color: "#3F9142",
  green8Color: "#327334",
  green9Color: "#255426",
  green10Color: "#183618",
  /** --------------------------------- Cyan -------------------------------- */
  cyan1Color: "#E6FFFB",
  cyan2Color: "#B5F5EC",
  cyan3Color: "#87E8DE",
  cyan4Color: "#5CDBD3",
  cyan5Color: "#36CFC9",
  cyan6Color: "#13C2C2",
  cyan7Color: "#08979C",
  cyan8Color: "#006D75",
  cyan9Color: "#00474F",
  cyan10Color: "#002329",
  /** --------------------------------- GeekBlue -------------------------------- */
  geekBlue1Color: "#F0F5FF",
  geekBlue2Color: "#D6E4FF",
  geekBlue4Color: "#85A5FF",
  geekBlue5Color: "#597EF7",
  geekBlue6Color: "#2F54EB",
  geekBlue7Color: "#1D39C4",
  geekBlue8Color: "#10239E",
  geekBlue9Color: "#061178",
  geekBlue10Color: "#030852",
  /** --------------------------------- Lime -------------------------------- */
  lime1Color: "#FCFFE6",
  lime2Color: "#F4FFB8",
  lime3Color: "#EAFF8F",
  lime4Color: "#D3F261",
  lime6Color: "#A0D911",
  lime7Color: "#7CB305",
  lime8Color: "#5B8C00",
  lime9Color: "#3F6600",
  lime10Color: "#254000",
  /** --------------------------------- Purple -------------------------------- */
  purple1Color: "#F9F0FF",
  purple2Color: "#EFDBFF",
  purple3Color: "#D3ADF7",
  purple4Color: "#B37FEB",
  purple5Color: "#9254DE",
  purple6Color: "#722ED1",
  purple7Color: "#531DAB",
  purple8Color: "#391085",
  purple9Color: "#22075E",
  purple10Color: "#120338",
  /** --------------------------------- Magenta -------------------------------- */
  magenta1Color: "#FFF0F6",
  magenta2Color: "#FFD6E7",
  magenta3Color: "#FFADD2",
  magenta4Color: "#FF85C0",
  magenta5Color: "#F759AB",
  magenta6Color: "#EB2F96",
  magenta7Color: "#C41D7F",
  magenta8Color: "#9E1068",
  magenta9Color: "#780650",
  magenta10Color: "#520339",
  /** --------------------------------- Graphite -------------------------------- */
  graphite0Color: "#383838",
  graphite1Color: "#2C2C2C",
  graphite2Color: "#232323",
  /** --------------------------------- Thrust -------------------------------- */
  thrust0Color: "#E1FAF6",
  thrust1Color: "#92DBD6",
  thrust2Color: "#50CCC4",
  thrust3Color: "#00E2D5",
  thrust4Color: "#0CB3B3",
  thrust5Color: "#089494",
  /** =========================== Colors [END]  =========================== */
};

export const theme = {
  // ----- padding && margin [START] -----//
  defaultSpace: 24,
  smallSpace: 12,
  topPanelHeight: 64,
  // ----- padding && margin [END] -----//

  // ----- tableStyles [START] -----//
  tableCheckboxColumnWidth: 38,
  tableContextMenuColumnWidth: 38,
  tableRowBorderSize: 1,
  tableRowLineHeight: 22,
  tableCellHorizontalPadding: 8,
  tableCellVerticalPadding: 8,
  tableSorterSize: 11,
  // ----- tableStyles [END] -----//

  // ----- editableTableStyles [START] -----//
  editableTableRowHeight: 42,
  editableTableFieldHeight: 28,
  editableTableFieldHorizontalPadding: 8,
  editableTableFieldVerticalPadding: 3,
  editableTableMarginLeft: 0,
  editableTableMarginRight: 8,
  // ----- editableTableStyles [END] -----//

  // ----- commonTableStyles [START] -----//
  /**
   * Минимальная высота строки для стандартных таблиц.
   * Общая высота строки формируется из вертикальных padding-отступов,
   * высоты контента и ширины границы, поэтому, при изменении высоты строки,
   * стоит учитывать все эти параметры.
   */
  commonTableRowHeight: 38,
  // ----- commonTableStyles [END] -----//

  // ----- drawerStyles [START] -----//
  drawerZIndex: 1050,
  drawerSmallWidth: 420,
  drawerMediumWidth: 580,
  drawerLargeWidth: 720,
  drawerXLargeWidth: 960,
  nextDrawerOverlap: 140,
  // ----- drawerStyles [END]-----//

  // ----- modalStyles [START] -----//
  modalLargeWidth: 720,
  // ----- modalStyles [END]-----//

  // ----- tooltipStyles [START] -----//
  maxTooltipWidth: 520,
  // ----- tooltipStyles [END]-----//

  // ----- fontSize [START] -----//
  h0FontSize: 40,
  h1FontSize: 24,
  h2FontSize: 22,
  h3FontSize: 18,
  subtitleFontSize: 16,
  h4FontSize: 14,
  h5FontSize: 12,
  h6FontSize: 10,
  // ----- fontSize [END] -----//

  // ----- lineHeight [START] -----//
  verySmallLineHeight: 20,
  defaultLineHeight: 22,
  smallLineHeight: 24,
  mediumLineHeight: 32,
  largeLineHeight: 40,
  // ----- lineHeight [END] -----//

  /** =========================== Colors Tags [START]  =========================== */
  tagsStyles: {
    blue: {
      backgroundColor: colors.blue1Color,
      borderColor: colors.blue15Color,
      textColor: colors.blue6Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    geekblue: {
      backgroundColor: colors.geekBlue1Color,
      borderColor: "#E6EDFA",
      textColor: colors.geekBlue7Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    red: {
      backgroundColor: colors.red1Color,
      borderColor: "#FAEAE6",
      textColor: colors.red6Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    volcano: {
      backgroundColor: colors.volcano1Color,
      borderColor: "#FAE9DC",
      textColor: colors.volcano6Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    orange: {
      backgroundColor: colors.orange1Color,
      borderColor: colors.orange11Color,
      textColor: colors.orange6Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    yellow: {
      backgroundColor: colors.yellow1Color,
      borderColor: "#F9FAD2",
      textColor: "#D4B106",
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    lime: {
      backgroundColor: colors.lime1Color,
      borderColor: "#F1F5D3",
      textColor: colors.lime7Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    green: {
      backgroundColor: colors.green1Color,
      borderColor: "#DDF0DE",
      textColor: colors.green6Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    cyan: {
      backgroundColor: colors.cyan1Color,
      borderColor: "#D7FAF4",
      textColor: colors.cyan6Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    pink: {
      backgroundColor: colors.magenta1Color,
      borderColor: "#FAE6EE",
      textColor: colors.magenta5Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    purple: {
      backgroundColor: colors.purple1Color,
      borderColor: "#F2E6FA",
      textColor: colors.purple7Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    grey: {
      backgroundColor: colors.grey3Color,
      borderColor: colors.grey4Color,
      textColor: colors.grey8Color,
      closeIconColor: colors.grey7Color,
      closeIconColorHover: colors.grey8Color,
    },
    darkGrey: {
      backgroundColor: colors.grey45Color,
      borderColor: colors.grey5Color,
      textColor: colors.grey8Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    blueInv: {
      backgroundColor: colors.blue6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    geekblueInv: {
      backgroundColor: colors.geekBlue7Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    redInv: {
      backgroundColor: colors.red6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    volcanoInv: {
      backgroundColor: colors.volcano6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    orangeInv: {
      backgroundColor: colors.orange6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    yellowInv: {
      backgroundColor: colors.yellow6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    limeInv: {
      backgroundColor: colors.lime6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    greenInv: {
      backgroundColor: colors.green6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    cyanInv: {
      backgroundColor: colors.cyan6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    pinkInv: {
      backgroundColor: colors.magenta5Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    purpleInv: {
      backgroundColor: colors.purple6Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
    greyInv: {
      backgroundColor: colors.grey7Color,
      borderColor: "transparent",
      textColor: colors.grey1Color,
      closeIconColor: colors.grey1Color,
      closeIconColorHover: colors.grey1Color,
    },
  },
  /** Header Menu */
  headerPanel: {
    backgroundColor: colors.graphite1Color,
    backgroundColorHover: colors.grey13Color,
    textColor: colors.grey3Color,
    textColorHover: colors.grey1Color,
    iconsBackgroundColor: colors.graphite1Color,
    iconsBackgroundColorHover: colors.grey13Color,
    iconsColor: colors.grey3Color,
    iconsColorHover: colors.grey1Color,
    dropdownBackgroundColor: colors.grey13Color,
    dropdownTextColor: colors.grey3Color,
    dropdownItemColorHover: colors.graphite1Color,
  },

  /** =========================== Colors Tags [END]  =========================== */

  /** =========================== Link Colors [START]  =========================== */

  linkHoverColor: colors.blue5Color,

  /** =========================== Link Colors [END]  =========================== */

  /** =========================== Brand Colors [START]  =========================== */

  brandDefaultColor: "#1E3799",
  brandDarkColor: "#05218C",

  /** =========================== Brand Colors [END]  =========================== */

  /** =========================== Colors [END]  =========================== */

  // ----- height [START] -----//
  // Высота меню сверху
  heightHeader: 56,
  heightHeaderMenu: 48,
  heightDividerHeaderMenu: 36,
  // ----- height [END] -----//
  ...colors,
} as const;
